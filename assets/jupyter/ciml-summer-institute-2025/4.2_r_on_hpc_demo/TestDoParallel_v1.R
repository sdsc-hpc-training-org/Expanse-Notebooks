#!/usr/bin/env Rscript
#A short exercise to run R in parallel and then view in top
...
library(doParallel)
registerDoParallel(cores=16) #<<<----- vary cores, match up to job resources
print('starting dopar test')

# Make up some random data and list out the object sizes
if (length(args)==0) {
      N=10000; #<<<<---- N rows start with say 1000
      P=200;   #<<<---   P columns  200 for start
} else {
      N=as.integer(args[1])
      P=as.integer(args[2])
      }
print(paste("Using N rows=",N," P cols=",P))
